def permute_bits(bits, permutation):
    """
    Permutes the bits according to the given permutation.

    Args:
        bits: A list of bits to permute.
        permutation: A list of indices specifying the permutation.

    Returns:
        A list of permuted bits.
    """
    return [bits[i] for i in permutation]

def left_shift(bits, shift_count):
    """
    Performs a left shift on the bits.

    Args:
        bits: A list of bits to shift.
        shift_count: The number of positions to shift.

    Returns:
        A list of shifted bits.
    """
    return bits[shift_count:] + bits[:shift_count]

def des_key_generation(key):
    """
    Generates a set of 16 subkeys for DES encryption.
